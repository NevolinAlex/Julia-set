...
function run(){
	...
	globals.Re = parseFloat(document.getElementById("Re").value);
	globals.Im = parseFloat(document.getElementById("Im").value);
	FractalType();
	StartAxis();
	init();
}
...
function init(){	
	...
	for (var i = 0; i < width; i++) {
		...
		for (var j = 0; j < height; j++) {
			...
			newZ.x = constComplex.x;
			newZ.y = constComplex.y;
			for (g = 0; g < n; g++){
				...
				var a = newZ.x;
				var b = newZ.y;
				newZ.x = a*a - b*b + globals.Re;
				newZ.y = 2*a*b + globals.Im;
			}
			if (g == n) 
